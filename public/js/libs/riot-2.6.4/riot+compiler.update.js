...
/**
 * Attach an event click delegate to a DOM node
 * @param { Function } handler - event callback
 * @param { Object } dom - dom node
 * @param { Tag } tag - tag instance
 * @param { Update } update - tag update
 */
// MY_FIXED
function setEventHandlerCustom(name, handler, dom, tag, update) {
  ...
  dom[name] = function(e) {
    ...
    update = dom.getAttribute("onupdate");

    if (!e.preventUpdate && update !== "none") {
      if (update === "self"){
        el = tag;
      }
      else if (update === "parent"){
        el = tag._parent ? tag._parent : tag
      }
      else {
...
function update(expressions, tag) {
  ...
  each(expressions, function(expr, i) {
    ...
    if (isFunction(value)) {
        // MY_FIXED
        if ((attrName === "onclick" || attrName === "onchange" || attrName === "oninput") && dom.getAttribute("onupdate")){
            setEventHandlerCustom(attrName, value, dom, tag, dom.getAttribute("onupdate"))
        }
		else {
